...
export default function Cart() {
  ...
  useEffect(() => {
          if (allimage) {
              const loadedImages = allimage.map(data => data.imageUrl);  // Direct Cloudinary URLs
  ...
  const filterdata = Array.isArray(data) ? data.filter((elem) => elem.userId == userid) : [];
          ...
          <table className='table'>
            ...
            <tbody>
              ...
              {logout && filterdata.map((elem, index) => (
                <>
                  <tr key={index}>
                    <td>  <img style={{  height: "4rem",width:"5rem" }} src={elem.image} alt="" srcset="" /></td>
                    <td>{elem.name}</td>
                    <td>{elem.category}</td>
                    <td>{elem.price}</td>
